def test_register_success(client, monkeypatch):
    ...
    resp = client.post(
        "/auth/register", json={"email": "leo@example.com", "password": "pass123"}
    )
...
def test_login_success_returns_token(client, monkeypatch):
    ...
    resp = client.post(
        "/auth/login", json={"email": "leo@example.com", "password": "pass123"}
    )
    assert resp.status_code == 200
...
def test_login_invalid_credentials(client, monkeypatch):
    ...
    resp = client.post(
        "/auth/login", json={"email": "leo@example.com", "password": "wrong"}
    )
